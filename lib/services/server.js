...
app.post('/login', (req, res) => {
  ...
  pool.query(sql, [emailOrPhone, emailOrPhone], (err, results) => {
    if (err) {
...
app.post('/signup', (req, res) => {
  ...
  pool.query(sql, [email, phone], (err, results) => {
    ...
    bcrypt.hash(password, 10, (err, hash) => {
      ...
      pool.query(insertSql, [name, surname, email, phone, hash], (err, results) => {
        if (err) {
...
app.post('/check_user_exists', (req, res) => {
  ...
  if (type === 'PhoneNumber' || type === 'phone') {
...
app.post('/send_otp', (req, res) => {
  const { type, value } = req.body;
  const otp = Math.floor(1000 + Math.random() * 9000).toString(); // สร้าง OTP 4 หลัก
...
app.post('/change_password', isAuthenticated, (req, res) => {
  ...
  console.log('User authenticated, proceeding to hash password:', user);
  ...
  bcrypt.hash(newPassword, 10, (err, hash) => {
    ...
    console.log('Password hashed successfully, updating in database:', hash);

    const sql = 'UPDATE user_information SET Password = ? WHERE UserID = ?';
    pool.query(sql, [hash, user.UserID], (err, results) => {
      if (err) {
